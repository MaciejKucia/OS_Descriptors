...
unsigned char g_pCDCSerDeviceDescriptor[] =
{
    ...
    USBShort(0x200),        // USB version 1.1 (if we say 2.0, hosts assume
                            // high-speed - see USB 2.0 spec 9.2.6.6) //TODO:
...
static void HandleDevice(void *pvInstance, unsigned long ulRequest,
                         void *pvRequestData);
//static void HandleOSDescriptor(void *pvInstance, unsigned long ulRequest,
//        void *pvRequestData);
...
HandleRequests(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    //
    // TODO: IF MS OS descriptor request
    //
	if (pUSBRequest->bmRequestType == 0xC0)
	{
		// TODO: there are other things to check here!
		if (pUSBRequest->wIndex == 4)
		{
			//MAP_USBDevEndpointDataAck(psInst->ulUSBBase, USB_EP_0, false);

			// is this header request or full descriptor?
			if (pUSBRequest->wLength == 0x10)
				USBDCDSendDataEP0(0, (unsigned char *)&g_pOSFeature, 0x10 );
			else
				USBDCDSendDataEP0(0, (unsigned char *)&g_pOSFeature, g_pOSFeature.dwLength );
	...
	//
	// TODO: IF extended MS OS descriptor request
	//
	else if (pUSBRequest->bmRequestType == 0xC1)
	{
